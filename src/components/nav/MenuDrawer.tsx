...
export function MenuDrawer() {
  ...
  useEffect(() => { close(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [pathname]);
  ...
  useEffect(() => { if (isOpen) { previouslyFocused.current = document.activeElement as HTMLElement | null; const id = requestAnimationFrame(() => closeButtonRef.current?.focus()); return () => cancelAnimationFrame(id); } previouslyFocused.current?.focus?.(); }, [isOpen]);
  useEffect(() => {
    if (!isOpen) return;
  ...
  const isActive = (href: string) => { const path = href.split('#')[0]; return path === '/' ? pathname === '/' : pathname.startsWith(path); };
...
function Section({ title, items, isOpen, isActive }: { title?: string; items: Item[]; isOpen: boolean; isActive: (href: string) => boolean; }) {
  return <div className="mb-1.5">{title && <h2 className="mb-1 px-2.5 pt-2 text-micro font-semibold uppercase tracking-[0.07em] text-ink-3">{title}</h2>}<ul>{items.map(({ href, label, icon: Icon, sub, badge }) => { const active = isActive(href); return <li key={label}><Link href={href} tabIndex={isOpen ? undefined : -1} aria-current={active ? 'page' : undefined} className={cn('flex items-center gap-3 rounded-[11px] px-2.5 py-2.5', active ? 'bg-blue-soft text-navy' : 'text-ink hover:bg-canvas')}><Icon size={19} strokeWidth={active ? 2.1 : 1.8} className={cn('shrink-0', active ? 'text-navy' : 'text-ink-2')} aria-hidden /><span className="min-w-0 flex-1"><span className={cn('block truncate text-[14.5px] tracking-[-0.015em]', active && 'font-semibold')}>{label}</span>{sub && <span className="mt-px block truncate text-caption text-ink-3">{sub}</span>}</span>{badge ? <span className="flex h-[18px] min-w-[18px] shrink-0 items-center justify-center rounded-lg bg-urgent px-1 text-[10px] font-bold text-white">{badge}</span> : null}</Link></li>; })}</ul></div>;
}
