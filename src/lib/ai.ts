...
/**
 * Summarises a tender from its first readable PDF using GroqCloud.
 */
...
async function generateSummary(tenderId: string): Promise<AiSummaryResult> {
  if (!isGroqConfigured()) {
    return {
      ...MOCK_SUMMARY,
      source: 'mock',
      notice: 'GROQ_API_KEY not set — showing a sample summary.',
    };
  }
...
/** Fallback when no document can be read. */
...
async function computeMatch(
  ...
  return { score, factors, warnings, source, model: GROQ_MODEL };
}
