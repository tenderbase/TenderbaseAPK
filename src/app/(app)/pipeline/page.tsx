...
type Stage = 'New match' | 'Review' | 'Bid planning' | 'Preparing' | 'Ready to submit' | 'Submitted' | 'Award pending' | 'Won' | 'Lost';
const STAGES: Stage[] = ['New match', 'Review', 'Bid planning', 'Preparing', 'Ready to submit', 'Submitted', 'Award pending', 'Won', 'Lost'];
...
export default function PipelinePage() {
  ...
  useEffect(() => { try { const raw = window.localStorage.getItem(STORAGE_KEY); if (raw) setStages(JSON.parse(raw) as Record<string, Stage>); } catch {} }, []);
  useEffect(() => { try { window.localStorage.setItem(STORAGE_KEY, JSON.stringify(stages)); } catch {} }, [stages]);
