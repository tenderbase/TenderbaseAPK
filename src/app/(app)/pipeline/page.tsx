...
import { ArrowRight, BriefcaseBusiness, CheckCircle2, Clock3, FileText, Plus, Target, Trophy } from 'lucide-react';
...
export default function PipelinePage() {
  ...
  const opportunities = useMemo(() => saved.map((row) => ({ tender: row.tender, stage: stages[row.tenderId] ?? 'Qualifying' as Stage })), [saved, stages]);
  const active = opportunities.filter((o) => o.stage !== 'Submitted');
  const totalValue = active.reduce((sum, o) => sum + (o.tender.valueCents ?? 0), 0);
  ...
  function move(id: string, current: Stage) {
    const next = STAGES[Math.min(STAGES.indexOf(current) + 1, STAGES.length - 1)];
    if (next !== current) setStages((prev) => ({ ...prev, [id]: next }));
  }
    ...
    <main className="tb-page">
      <header className="tb-content pt-6 sm:pt-8">
        <div className="flex items-start justify-between gap-4">
          ...
          <span className="tb-icon-button"><Plus size={20} strokeWidth={1.9} aria-hidden /></span>
        </div>
      </header>

      <section className="tb-content mt-6 grid grid-cols-3 gap-2.5">
        <Stat icon={<Target size={16} />} label="Active bids" value={String(active.length)} />
        <Stat icon={<Trophy size={16} />} label="Pipeline value" value={formatValue(totalValue || null)} />
        ...
        {opportunities.length === 0 ? (
          ...
          <div className="grid gap-4 xl:grid-cols-4">
            {STAGES.map((stage) => {
              ...
              return (
                <div key={stage} className="rounded-[16px] border border-line bg-white p-3">
                  <div className="flex items-center justify-between px-1 pb-3"><div className="flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-canvas text-ink-2 text-[10px] font-bold">{STAGES.indexOf(stage) + 1}</span><h2 className="text-[13px] font-semibold text-ink">{stage}</h2></div><span className="text-[11px] font-medium text-ink-3">{items.length}</span></div>
                  <div className="space-y-2.5">
                    {items.length === 0 ? <div className="rounded-[12px] border border-dashed border-line px-3 py-5 text-center text-[11px] text-ink-4">No opportunities</div> : items.map(({ tender }) => (
                      <article key={tender.id} className="rounded-[13px] border border-line bg-canvas p-3">
                        <Link href={`/tenders/${tender.id}`} className="block"><p className="line-clamp-3 text-[12.5px] font-semibold leading-[1.35] text-ink">{tender.title}</p><p className="mt-1 text-[10.5px] text-ink-3">{tender.organisation}</p></Link>
                        <div className="mt-2 flex items-center gap-2 text-[10px] text-ink-3"><span className="inline-flex items-center gap-1"><FileText size={11} />{tender.documents.length}</span>{tender.valueCents !== null && <span>{formatValue(tender.valueCents)}</span>}</div>
                        {stage !== 'Submitted' && <button type="button" onClick={() => move(tender.id, stage)} className="mt-2 flex h-8 w-full items-center justify-center gap-1 rounded-[9px] bg-white text-[10.5px] font-semibold text-navy">Move to {STAGES[Math.min(STAGES.indexOf(stage) + 1, STAGES.length - 1)]}<ArrowRight size={12} /></button>}
...
function Stat({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="tb-card p-3.5"><div className="flex items-center gap-2 text-ink-3">{icon}<span className="text-[9px] font-bold uppercase tracking-[.08em]">{label}</span></div><p className="mt-1.5 truncate text-[18px] font-bold text-ink">{value}</p></div>;
}
