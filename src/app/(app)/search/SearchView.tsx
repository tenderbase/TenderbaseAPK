...
import { SlidersHorizontal, ArrowUpDown, SearchX, Loader2, CloudOff, BookmarkPlus, Check, Target } from 'lucide-react';
...
export function SearchView({ results, total, page, totalPages, source, notice, initialQuery, activeSort }: SearchViewProps) {
  ...
  useEffect(() => {
    const hasFilters = ['q', 'category', 'province', 'status', 'closingWithin'].some((k) => params.get(k));
  ...
  const activeQuick = (f: (typeof QUICK_FILTERS)[number]) => {
    ...
    if (entries.length === 0) return !params.get('category') && !params.get('province') && !params.get('closingWithin') && !params.get('status');
    ...
    <main className="pb-24">
      <header className="sticky top-0 z-30 border-b border-line bg-white px-5 pb-3.5 pt-2">
        ...
        <div className="mt-3 grid grid-cols-3 gap-2">
          <button type="button" onClick={() => setBestFit((v) => !v)} aria-pressed={bestFit} className={`flex h-9 items-center justify-center gap-1.5 rounded-[10px] border px-2 text-meta font-semibold ${bestFit ? 'border-ai-line bg-ai-bg text-ai' : 'border-line bg-white text-ink'}`}><Target size={15} strokeWidth={2} aria-hidden /> Best fit</button>
          <button type="button" onClick={() => { setBestFit(false); update({ sort: activeSort === 'closing_soon' ? 'newest' : 'closing_soon' }); }} className="flex h-9 items-center justify-center gap-1.5 rounded-[10px] border border-line bg-white text-meta font-medium text-ink"><ArrowUpDown size={15} strokeWidth={2} aria-hidden /> {activeSort === 'closing_soon' ? 'Closing soon' : 'Newest'}</button>
