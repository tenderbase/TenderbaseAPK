...
import type { MunicipalityFacet, ProcurementTypeFacet } from '@/lib/municipality-facets.server';
...
export function SearchView({
  ...
  const rankedResults = useMemo(() => {
    const rows = shownResults.map((t) => {
      const score = scored.get(t.id) ?? 0;
      return score > 0 ? { ...t, matchScore: score } : t;
    });
    ...
    return [...rows].sort((a, b) => {
      ...
      return new Date(a.closingDate || 0).getTime() - new Date(b.closingDate || 0).getTime();
    });
  ...
  const toggleQuick = (f: (typeof QUICK_FILTERS)[number]) => {
    if (Object.keys(f.params).length === 0) {
      update({ category: undefined, province: undefined, closingWithin: undefined, status: undefined, municipality: undefined, municipalityCode: undefined, procurementType: undefined });
      return;
    }
  ...
  return (
    <main className="pb-24">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
          <label className="flex h-10 min-w-0 items-center gap-2 rounded-[10px] border border-line bg-white px-3">
            ...
            <select
              ...
              value={params.get('municipalityCode') ?? params.get('municipality') ?? ''}
              onChange={(e) => update({ municipalityCode: e.target.value || undefined, municipality: undefined })}
          ...
          <label className="flex h-10 min-w-0 items-center gap-2 rounded-[10px] border border-line bg-white px-3">
            ...
            <select
              ...
              value={params.get('procurementType') ?? ''}
              onChange={(e) => update({ procurementType: e.target.value || undefined })}
              className="min-w-0 flex-1 bg-transparent text-[12px] font-semibold text-ink outline-none"
            >
      ...
      <div className="px-5 pt-3.5">
        ...
        {(params.get('municipalityCode') || params.get('municipality')) && (
          <div className="mb-3 flex items-center gap-2 rounded-[12px] border border-blue-line bg-blue-soft px-3.5 py-2.5">
            <MapPin size={15} className="shrink-0 text-blue" aria-hidden />
            <p className="text-[12px] text-ink-2"><span className="font-semibold text-ink">Municipality filter active.</span> Results are restricted to the selected local authority.</p>
          </div>
        )}
        {params.get('procurementType') && (
          <div className="mb-3 flex items-center gap-2 rounded-[12px] border border-pro-line bg-pro-soft px-3.5 py-2.5">
            <FileText size={15} className="shrink-0 text-[#7a610f]" aria-hidden />
            <p className="text-[12px] text-ink-2"><span className="font-semibold text-ink">{params.get('procurementType')} filter active.</span> The API is applying this procurement class before results reach the UI.</p>
          </div>
        )}
