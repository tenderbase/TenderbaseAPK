...
import {
  ...
  Sparkles, CircleCheck, Circle, MessageSquare, CalendarDays, MoreHorizontal,
...
function Section({
  ...
  return (
    <section className={cn('rounded-[18px] border border-line bg-white p-4 sm:p-5', className)}>
      <div className="mb-4 flex items-end justify-between gap-3">
        ...
        {count !== undefined && <span className="text-[12px] font-medium text-ink-3">{count}</span>}
      </div>
      {children}
    </section>
  );
...
export function TenderDetailView({
  ...
  return (
    <main className="min-h-screen bg-canvas pb-[180px] md:pb-28">
      {/* Quiet navigation chrome */}
      <header className="sticky top-0 z-30 border-b border-line/80 bg-white/92 px-4 py-2.5 backdrop-blur-xl sm:px-6">
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <section className="relative overflow-hidden rounded-[24px] bg-navy px-5 pb-5 pt-6 text-white shadow-[0_18px_50px_rgba(9,27,49,.16)] sm:px-7 sm:pb-7 sm:pt-7">
          ...
          <div className="relative">
            ...
            <h1 className="mt-4 max-w-3xl text-[25px] font-semibold leading-[1.15] tracking-[-0.035em] sm:text-[32px]">
              {tender.title}
            </h1>
            <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2 text-[13px] text-white/65">
              <span className="inline-flex items-center gap-1.5"><Building2 size={15} />{tender.organisation}</span>
              {tender.location && <span className="inline-flex items-center gap-1.5"><MapPin size={15} />{tender.location}</span>}
            </div>

            <div className="mt-6 grid grid-cols-3 divide-x divide-white/10 rounded-[16px] border border-white/10 bg-white/[.055]">
              <div className="px-3 py-3.5 sm:px-4">
                <p className="text-[9px] font-semibold uppercase tracking-[.12em] text-white/45">Deadline</p>
                <p className="mt-1.5 text-[14px] font-semibold sm:text-[15px]">{tender.closingDate ? formatDate(tender.closingDate) : 'Not stated'}</p>
              </div>
              <div className="px-3 py-3.5 sm:px-4">
                <p className="text-[9px] font-semibold uppercase tracking-[.12em] text-white/45">Time left</p>
                <p className={cn('mt-1.5 text-[14px] font-semibold sm:text-[15px]', urgency ? 'text-[#ffadad]' : 'text-[#f2c06b]')}>
                  {remaining === null ? '—' : remaining < 0 ? 'Closed' : remaining === 0 ? 'Today' : `${remaining} days`}
                </p>
              </div>
              <div className="px-3 py-3.5 sm:px-4">
                <p className="text-[9px] font-semibold uppercase tracking-[.12em] text-white/45">Opportunity</p>
                <p className={cn('mt-1.5 font-semibold sm:text-[15px]', tender.valueCents === null ? 'text-[12px]' : 'text-[14px]')}>{formatValue(tender.valueCents)}</p>
              </div>
            </div>
        ...
        {/* Pipeline context */}
        <section className="mt-3 rounded-[18px] border border-line bg-white p-4 sm:p-5">
          <div className="flex items-center justify-between gap-4">
            ...
            <button type="button" className="hidden items-center gap-1.5 rounded-[10px] px-3 py-2 text-[12px] font-semibold text-blue hover:bg-blue-soft sm:flex">
          ...
          <div className="mt-4 flex items-center gap-1.5 overflow-hidden">
            {['Discover', 'Qualify', 'Pursue', 'Prepare', 'Submit'].map((stage, i) => (
              <div key={stage} className="flex min-w-0 flex-1 items-center gap-1.5">
                <span className={cn('h-1.5 flex-1 rounded-full', i === 0 ? 'bg-blue' : 'bg-line')} />
                {i < 4 && <span className="hidden text-[9px] text-ink-3 sm:block">{stage}</span>}
              </div>
            ))}
          </div>
        </section>

        <div className="mt-3 grid gap-3 lg:grid-cols-[minmax(0,1.35fr)_minmax(300px,.65fr)]">
          <div className="space-y-3">
            ...
            <Section eyebrow="Next right move" title="Decide if this is worth pursuing" className="border-blue/15 bg-[linear-gradient(180deg,#fff,#fbfdff)]">
              <div className="rounded-[14px] border border-blue/12 bg-blue-soft/45 p-4">
            ...
            <Section eyebrow="Source record" title="Overview">
              <dl className="grid gap-x-8 sm:grid-cols-2">
                {([
                  ...
                  <div key={k} className="flex items-start justify-between gap-4 border-b border-line py-3 first:pt-0 last:border-b-0 sm:nth-last-2:border-b-0">
                    <dt className="text-[12.5px] text-ink-2">{k}</dt>
                    <dd className="max-w-[62%] text-right text-[12.5px] font-semibold text-ink">{v}</dd>
                  </div>
                ))}
              </dl>
              {tender.description && (
                <div className="mt-4 border-t border-line pt-4">
                  ...
                  <p className="mt-2 whitespace-pre-line text-[13px] leading-6 text-ink-2">{normaliseCase(tender.description)}</p>
                </div>
              )}
            </Section>
            ...
            {tender.contactInformation && (
              <Section eyebrow="Issuer" title="Contact">
                <div className="divide-y divide-line">
                  {tender.contactInformation.contactPerson && <div className="flex items-center gap-3 py-2.5"><span className="flex h-8 w-8 items-center justify-center rounded-[9px] bg-canvas text-ink-3"><User size={15} /></span><span className="text-[13px] font-semibold text-ink">{tender.contactInformation.contactPerson}</span></div>}
                  {tender.contactInformation.email && <a href={`mailto:${tender.contactInformation.email}`} className="flex items-center gap-3 py-2.5"><span className="flex h-8 w-8 items-center justify-center rounded-[9px] bg-canvas text-ink-3"><Mail size={15} /></span><span className="truncate text-[13px] font-medium text-blue">{tender.contactInformation.email}</span></a>}
                  {tender.contactInformation.phone && <a href={`tel:${tender.contactInformation.phone.replace(/\s+/g, '')}`} className="flex items-center gap-3 py-2.5"><span className="flex h-8 w-8 items-center justify-center rounded-[9px] bg-canvas text-ink-3"><Phone size={15} /></span><span className="text-[13px] font-medium text-blue">{tender.contactInformation.phone}</span></a>}
                </div>
              </Section>
            )}
          </div>
          ...
          <aside className="space-y-3">
            ...
            <Section eyebrow="Submission readiness" title="Work to complete">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex h-14 w-14 items-center justify-center rounded-full border-[5px] border-line text-[11px] font-semibold text-ink-2">0 / 0</div>
              </div>
            ...
            <Section eyebrow="Your team" title="Keep the opportunity moving">
              <div className="flex -space-x-2">
                {['S','M','J'].map((initial) => <span key={initial} className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-white bg-navy text-[10px] font-semibold text-white">{initial}</span>)}
                <button type="button" className="ml-2 flex h-8 items-center gap-1 rounded-full bg-canvas px-3 text-[10px] font-semibold text-ink-2"><span>+ Add</span></button>
              </div>
              <div className="mt-4 space-y-2">
                <div className="flex items-center gap-2.5 rounded-[11px] bg-canvas px-3 py-2.5"><Clock3 size={14} className="text-ink-3" /><span className="text-[11.5px] text-ink-2">Deadline reminder</span><span className="ml-auto text-[10px] font-semibold text-ink">Set</span></div>
                <div className="flex items-center gap-2.5 rounded-[11px] bg-canvas px-3 py-2.5"><MessageSquare size={14} className="text-ink-3" /><span className="text-[11.5px] text-ink-2">Tender conversation</span><span className="ml-auto text-[10px] font-semibold text-blue">Open</span></div>
              </div>
            </Section>

            <Section eyebrow="Timeline" title="Recent activity">
              <div className="space-y-4">
                <div className="flex gap-3"><span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-blue" /><div><p className="text-[12px] font-medium text-ink">Tender added to TenderBase</p><p className="mt-0.5 text-[10.5px] text-ink-3">Today</p></div></div>
                <div className="flex gap-3"><span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-line" /><div><p className="text-[12px] font-medium text-ink">Source record available</p><p className="mt-0.5 text-[10.5px] text-ink-3">Source verified</p></div></div>
              </div>
            </Section>
          </aside>
        </div>
        ...
        <div className="mt-3 grid gap-3 lg:grid-cols-2">
            ...
            {tender.documents.length === 0 ? (
              ...
              <ul className="space-y-2">
                {tender.documents.map((d) => {
                  ...
                  return <li key={d.id}><a href={d.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 rounded-[13px] border border-line px-3 py-3 transition-colors hover:bg-canvas"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] bg-canvas text-navy"><FileText size={16} /></span><span className="min-w-0 flex-1"><span className="block truncate text-[12.5px] font-semibold text-ink">{d.name}</span><span className="mt-0.5 block text-[10px] text-ink-3">{d.fileType}{size ? ` · ${size}` : ''}{d.isAddendum ? ' · Addendum' : ''}</span></span><Download size={16} className="shrink-0 text-ink-3" /></a></li>;
                })}
              </ul>
            )}
          </Section>
          ...
          <Section eyebrow="Change tracking" title="Amendments" count={amendments.length}>
            {amendments.length === 0 ? (
              ...
              <ul className="space-y-2">{amendments.map((a) => <li key={a.id} className="rounded-[13px] border border-warning/20 bg-warning/5 px-3.5 py-3"><p className="text-[12.5px] font-semibold text-ink">{a.field} changed</p><p className="mt-1 text-[11px] text-ink-2">{a.from ?? '—'} → <span className="font-semibold text-ink">{a.to ?? '—'}</span></p><p className="mt-1 text-[9.5px] text-ink-3">Detected {formatDate(a.detectedAt)}</p></li>)}</ul>
            )}
          </Section>
        </div>
      </div>

      {/* Primary workspace actions remain reachable without competing with the page */}
      <footer className="fixed inset-x-0 bottom-0 z-40 border-t border-line bg-white/94 px-4 pb-[calc(12px+env(safe-area-inset-bottom))] pt-2.5 backdrop-blur-xl md:left-60 md:pb-3">
        <div className="mx-auto flex max-w-6xl gap-2.5">
          {signedIn && <button type="button" onClick={() => setFollowOpen(true)} className="flex h-12 flex-1 items-center justify-center gap-2 rounded-[12px] border border-line bg-white text-[13px] font-semibold text-ink transition-colors hover:bg-canvas"><Bell size={16} /> Follow</button>}
          <button type="button" onClick={() => toggleSaved(tender)} aria-pressed={saved} className={cn('flex h-12 flex-[1.35] items-center justify-center gap-2 rounded-[12px] text-[13px] font-semibold text-white transition-all active:scale-[.99]', saved ? 'bg-success' : 'bg-navy')}><Bookmark size={16} fill={saved ? 'currentColor' : 'none'} />{saved ? 'Saved to your workspace' : 'Save opportunity'}</button>
        </div>
      </footer>
