...
export function TenderDecisionStrip({ tender }: { tender: TenderWithUserState }) {
  ...
  function setStage(stage: string) {
    try { localStorage.setItem(`tenderbase-stage-${tender.id}`, stage); window.location.reload(); } catch {}
  }
  ...
  return (
    <section className={`mx-auto mb-3 mt-3 flex w-full max-w-6xl flex-col gap-3 rounded-[18px] border px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5 ${tone}`}>
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-2">
        ...
        {recommendation !== 'Skip' && <button type="button" onClick={() => setStage('Pursuing')} className="inline-flex h-9 items-center gap-1.5 rounded-[10px] bg-navy px-3 text-[11.5px] font-semibold text-white">Move to pursuing <ArrowRight size={13}/></button>}
      </div>
    </section>
  );
}
